...
exports.handler = async (event) => {    
    try{
        logger.log("index.js", 'agentprofile', event, "info");
        const {emailAddress} = JSON.parse(event.body);
        const userRecordQuery = `SELECT * ${config.table} FROM  WHERE emailAddress='${emailAddress}'`;
        const {
            data :{
                0: userRecord
            }
        ...
        // }
        return createResponse(200, userRecord);
    } catch (error) {
        logger.log("index.js", 'agentprofile', error.message, "info");
